...
class Page {
    ...
    public function __construct($totalRows,$listRows='',$parameter='',$url='') {  
        ...
        $this->varPage      =   config('admin_page.var_page') ? config('admin_page.var_page') : 'p';
        if(!empty($listRows)) {  
            $this->listRows =   intval($listRows);  
        }  
        $this->totalPages   =   ceil($this->totalRows/$this->listRows);     //总页数  
        $this->coolPages    =   ceil($this->totalPages/$this->rollPage);  
        $request = Request::instance()->param($this->varPage); //获取当前页数
        $this->nowPage      =   !empty($request)?intval($request):1;
        if($this->nowPage<1){  
            $this->nowPage  =   1;  
