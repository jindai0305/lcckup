...
function GenerateComments(queryData, anchorflag, anchorRootCmtId,anchorNewCmtId, anchorcmtId) {
    ...
    $.ajax({
        type: "GET",
        url: "/Home/Comment/CommList/id/" + tid +queryData,
        beforeSend: function () {
            ...
            if (data.success) {
				...
				if(data.CoreData!=""){
                    ...
                    if (data.PageNavStr != "") {
                        ...
                        jQuery('.comments-wrap #comments .pagination').css("text-align", "right");
                        //为评论分页按钮 绑定 click事件
                        jQuery('.pagination ul a').click(function () {
                            ...
                            jQuery('.comments-wrap #comments .commentlist').remove(); //直接将所有评论的大容器去掉
                            jQuery('#loading-comments').slideDown(); //缓冲图标缓慢展示
                            jQuery('html,body').animate({ scrollTop: $('#comments-title').offset().top - 80 }, 800); //65是"n条评论的高度"
                ...
                if (anchorflag == "1") {
                    if(anchorRootCmtId=="1"){
                        //定位到评论区
                        jQuery('html,body').animate({ scrollTop: $('#comment-' + anchorcmtId).offset().top - 110 }, 800);
						...
						jQuery('html,body').animate({ scrollTop: $('#second-comment-' + anchorcmtId).offset().top - 110 }, 800);
...
function Comment(type,sort){
    ...
    jQuery.ajax({
        type: "POST",
        url: "/Home/ArtList/AddNewAss",
        data:{'VId':VId,'TId':TId,'VName':VName,'VEmail':VEmail,'CmtText':CmtText,'type':type},
        dataType: "json",
        beforeSend: function () {
...
function addComment(id,tid,type,sort){
    ...
    html_c+='<i class="fa fa-pencil"></i>快来评论';
    html_c+='<small><a rel="nofollow" id="closeCom" href="javascript:;">取消回复</a>';
    html_c+='</small></h3>';
    ...
    html_c+='<input id="tid" name="tId" value='+tid+' type="hidden">';
    html_c+='<div class="row">';
    html_c+='<div class="col-md-6" style="float: none">';
...
function resetComm(html){
    ...
    jQuery("#submit").bind('click', function(){Comment(0)});//为按钮绑定提交函数
}
